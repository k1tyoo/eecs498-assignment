...
def mutate_tensor(
    ...
    for idx in range(len(indices)):
        x[indices[idx][0], indices[idx][1]] = values[idx] 
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    if num == 0:
        ...
        x = torch.tensor([10 * i for i in range(1, num+1)], dtype=torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def shuffle_cols(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    y = torch.zeros((x.shape[0], 4)).to(x)
    y[:, [0, 1]] = x[:, [0, 0]]
    y[:, 2] = x[:, 2]
    y[:, 3] = x[:, 1]
    ##########################################################################
...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    y[[i for i in range(x.shape[0])], :] = x[[i for i in range(x.shape[0] - 1, -1, -1)], :]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    row = len(x)
    col = max(x) + 1
    y = torch.zeros((row, col)).scatter_(1, torch.unsqueeze(torch.tensor(x), 1), 1)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = x.clone()
    indices = torch.argmin(y, dim=1, keepdim=True)
    y.scatter_(1, indices, 0)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    b, n, p = x.shape[0], x.shape[1], y.shape[2]
    z = torch.zeros((b, n, p)).to(x)
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    m = x.shape[1]
    mu = torch.sum(x, dim=0) / m
    sigma = torch.sqrt(torch.sum((x - mu).pow(2), dim=0) / (m - 1))
    y = (x - mu) / sigma
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
