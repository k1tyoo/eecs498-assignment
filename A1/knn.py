...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        for j in range(num_test):
            dists[i, j] = torch.sqrt((x_train[i] - x_test[j]).pow(2).sum())
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_test = x_test.view(num_test, -1)
    for i in range(num_train):
        dists[i] = torch.sqrt((x_train[i] - x_test).pow(2).sum(dim=1))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train = x_train.view(num_train, -1)
    x_test = x_test.view(num_test, -1)
    x_train_square = x_train.pow(2).sum(dim=1, keepdim=True)
    x_test_square = x_test.pow(2).sum(dim=1, keepdim=True).view(1, num_test)
    xx = torch.matmul(x_train, x_test.T)
    dists = torch.sqrt(x_train_square + x_test_square - 2*xx)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
        ...
        labels = y_train[indices].to(torch.int).view(-1)
        # print(labels.shape)
...
def knn_cross_validate(
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
            ...
            x_tra = x_train_folds[:i] + x_train_folds[i+1:]
            y_tra = y_train_folds[:i] + y_train_folds[i+1:]
            x_tra = torch.cat(x_tra)
            y_tra = torch.cat(y_tra)    
            KNN = KnnClassifier(x_tra, y_tra)
            acc = KNN.check_accuracy(x_val, y_val, k, True)
